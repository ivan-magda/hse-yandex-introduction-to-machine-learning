...
Метод главных компонент решает задачу понижения размерности - он находит новые признаки, зависящие от старых. При этом целевая переменная никак не используется, поэтому речь не идет ни о регрессии, ни о классификации.
